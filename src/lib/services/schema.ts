...
const getDefinedTermSetData = (content: DictionaryEntry[]): SchemaDefinition => {
  ...
  return {
    ...
    name: `Viking Language Dictionary - Letter ${letter.letter.toUpperCase()}`,
    description: `Meanings of common Old Norse words starting with letter ${letter.letter.toUpperCase()}`,
  }
}
...
const getDefinedTermData = (content: DictionaryEntry): SchemaDefinition => ({
  ...
  name: `Viking Language Dictionary - ${capitalize(content.headword)}`,
